...
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/etc/xray/xrayvmesstls.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo -e ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}                Memadam Akaun Xray Vmess                  ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
...
sed -i "/^### $user $exp/,/^},{/d" /etc/xray/xrayvmesstls.json
sed -i "/^### $user $exp/,/^},{/d" /etc/xray/xrayvmessnonetls.json
sed -i "/^### $user $exp/,/^},{/d" /etc/xray/xrayvmessmkcp.json
...
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}              Akaun XrayVmess Berjaya Dipadam             ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e ""

...
NUMBER_OF_CLIENTS=$(grep -c -E "^#&# " "/etc/xray/xraytrojangrpc.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo -e ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}             Memadam Akaun Xray Trojan Grpc               ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
...
CLIENT_NAME=$(grep -E "^#&# " "/etc/xray/xraytrojangrpc.json" | cut -d ' ' -f 2-3 | sed -n "${CLIENT_NUMBER}"p)
...
sed -i "/^#&# $user $exp/,/^},{/d" /etc/xray/xraytrojangrpc.json
sed -i "/^#&# $user $exp/,/^},{/d" /etc/xray/xraytrojangrpc.json
systemctl restart xraytrojangrpc.service
service cron restart
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}           Akaun Xray Trojan GrpcBerjaya Dipadam          ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e ""

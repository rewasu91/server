...
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/var/lib/Kaizen/data-user-l2tp")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo -e ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}                   Memadam Akaun L2TP                     ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
	...
	VPN_USER=$(grep -E "^### " "/var/lib/Kaizen/data-user-l2tp" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
...
sed -i "/^### $VPN_USER $exp/d" /var/lib/Kaizen/data-user-l2tp
# Update file attributes
chmod 600 /etc/ppp/chap-secrets* /etc/ipsec.d/passwd*
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}                Akaun L2TP Berjaya Dipadam                ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e ""

...
data=( `cat /etc/xray/xrayxtls.json | grep '^#DEPAN' | cut -d ' ' -f 4`);
echo "-------------------------------------";
echo "Name : Xray Vless Tcp Xtls User Login";
echo "-------------------------------------";
for akun in "${data[@]}"
do
if [[ -z "$akun" ]]; then
...
for ip in "${data2[@]}"
do
jum=$(cat /var/log/xray/access_xrayxtls.log | grep -w $akun | awk '{print $3}' | cut -d: -f1 | grep -w $ip | sort | uniq)
if [[ "$jum" = "$ip" ]]; then

...
NUMBER_OF_CLIENTS=$(grep -c -E "^#&# " "/etc/xray/xraytrojan.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}             Perbaharui Akaun Xray Trojan                 ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
		echo "► Tiada akaun Trojan yang aktif. Sila buat akaun!"
		echo -e ""
		exit 1
	fi
	clear
...
sed -i "s/#&# $user $exp/#&# $user $exp4/g" /etc/xray/xraytrojan.json
systemctl restart xraytrojan.service
clear
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}  Maklumat Akaun Xray Trojan Yang Sudah Diperbaharui      ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"

...
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/etc/xray/xrayvmesstls.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		clear
		echo ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}             Perbaharui Akaun Xray Vmess                  ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
		echo "► Tiada akaun Xray Vmess yang aktif. Sila buat akaun!"
		exit 1
	fi
	...
	echo -e "==============================="
	grep -E "^#### " "/etc/xray/xrayvmesstls.json" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^#### " "/etc/xray/xrayvmesstls.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#### " "/etc/xray/xrayvmesstls.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
sed -i "s/### $user $exp/### $user $exp4/g" /etc/xray/xrayvmesstls.json
sed -i "s/### $user $exp/### $user $exp4/g" /etc/xray/xrayvmessnonetls.json
sed -i "s/### $user $exp/### $user $exp4/g" /etc/xray/xrayvmessmkcp.json
...
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}    Maklumat Akaun Xray Vmess Yang Sudah Diperbaharui     ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"

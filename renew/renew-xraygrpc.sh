...
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/etc/xray/xrayvmessgrpc.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}             Perbaharui Akaun Xray Grpc                   ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
...
sed -i "s/### $user $exp/### $user $exp4/g" /etc/xray/xrayvmessgrpc.json
sed -i "s/### $user $exp/### $user $exp4/g" /etc/xray/xrayvlessgrpc.json
...
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}    Maklumat Akaun Xray Grpc Yang Sudah Diperbaharui      ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"

...
NUMBER_OF_CLIENTS=$( grep -c -E "^#DEPAN " "/etc/xray/xrayxtls.json" )
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}          Perbaharui Akaun Xray Vless XTLS                ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
		echo "► Tiada akaun Xray Vless XTLS yang aktif. Sila buat akaun!"
		exit 1
	fi
...
user=$( grep -E "^#DEPAN " "/etc/xray/xrayxtls.json" | cut -d ' ' -f 4 | sed -n "${CLIENT_NUMBER}"p)
exp=$( grep -E "^#DEPAN " "/etc/xray/xrayxtls.json" | cut -d ' ' -f 8 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}  Maklumat Akaun Xray Vless XTLS Yang Sudah Diperbaharui  ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"

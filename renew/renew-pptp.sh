...
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/var/lib/Kaizen/data-user-pptp")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		...
		echo ""
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e "${D1}                Perbaharui Akaun PPTP                     ${R}"
		echo -e "${D}——————————————————————————————————————————————————————————${R}"
		echo -e ""
...
user=$(grep -E "^### " "/var/lib/Kaizen/data-user-pptp" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
...
sed -i "s/### $user $exp/### $user $exp4/g" /var/lib/Kaizen/data-user-pptp
clear
echo -e ""
echo -e "${D}——————————————————————————————————————————————————————————${R}"
echo -e "${D1}      Maklumat Akaun PPTP Yang Sudah Diperbaharui         ${R}"
echo -e "${D}——————————————————————————————————————————————————————————${R}"
